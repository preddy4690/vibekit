...
export async function generateCode({
  ...
  try {
    ...
    // Build conversation history from task messages - SIMPLE VERSION
    ...
    console.log(`[GENERATE_CODE] Conversation history: ${conversationMessages.length} messages`);
      ...
      callbacks: {
        ...
        try {
          ...
          if (parsedMessage.type === 'local_shell_call') {
            ...
            // Final AI response - CLEAN TEXT EXTRACTION
            let text = '';
    ...
    // SIMPLE PR creation logic - only for CODE mode
    if (task.mode === 'code') {
